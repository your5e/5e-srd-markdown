...
@test "statblock - must start with header" {
    ...
    run fix_statblock_headers "$BATS_TEST_TMPDIR/no_headers.md"
...
@test "statblock - fix known headers" {
    ...
    run fix_statblock_headers "$BATS_TEST_TMPDIR/statblock.md"
...
@test "statblock - don't fix unknown headers" {
    ...
    run fix_statblock_headers "$BATS_TEST_TMPDIR/statblock_error.md"
    diff -u <(echo "$expected_output") <(echo "$output")
...
@test "generic - fix headers that flow" {
    ...
    run fix_headers "$BATS_TEST_TMPDIR/progression.md"
    diff -u <(echo '') <(echo "$output")
...
@test "generic - headers must progress" {
    ...
    run fix_headers "$BATS_TEST_TMPDIR/regression.md"
    diff -u <(echo "$expected_output") <(echo "$output")
...
@test "generic - must start with header" {
    ...
    run fix_headers "$BATS_TEST_TMPDIR/no_headers.md"
    diff -u <(echo "$expected_output") <(echo "$output")
    diff -u tests/headers/no_headers.md "$BATS_TEST_TMPDIR/no_headers.md"
    [ "$status" -eq 1 ]
}
...
@test "generic - valid file no changes" {
    ...
    run fix_headers "$BATS_TEST_TMPDIR/valid.md"
    diff -u <(echo '') <(echo "$output")
...
@test "generic - jumps file unchanged" {
    ...
    run fix_headers "$BATS_TEST_TMPDIR/jumps.md"
    diff -u <(echo '') <(echo "$output")
...
@test "generic - statblock file errors" {
    ...
    run fix_headers "$BATS_TEST_TMPDIR/statblock.md"
    diff -u <(echo "$expected_output") <(echo "$output")
