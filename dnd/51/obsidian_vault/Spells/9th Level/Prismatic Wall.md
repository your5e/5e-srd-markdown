...
The wall can be destroyed, also one layer at a time, in order from red to violet, by means specific to each layer. Once a layer is destroyed, it remains so for the duration of the spell. A _rod of cancellation_ destroys a _Prismatic Wall_, but an [[Antimagic Field]] has no effect on it.
...
4. _**Green.**_ The creature takes 10d6 poison damage on a failed save, or half as much damage on a successful one. A [[Passwall]] spell, or another spell of equal or greater level that can open a portal on a solid surface, destroys this layer.
